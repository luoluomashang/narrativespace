...
### ⛔ 零步骤：强制上下文重载（每章开始必须执行，不可跳过）

> **作用**：对抗长上下文注意力衰减（失忆）。无论当前对话已持续多长时间，进入新章的 INIT 时必须从零执行此区块。
...
步骤 C：内部状态确认（必须在主进程输出此 token，不得省略）
  → 格式：[上下文已重载 | 第{N}章 | 模式:interactive | tension={X} | 规则已激活]
```

**⚠ 警告：未完成步骤 A~C，禁止进入 INIT 执行清单任何后续步骤。**
...
6. **一次性编译 write_constraints**（≤500 字摘要）：
   ...
   - 扫描 `.xushikj/config/style_modules/clone_*.yaml`：若存在，加载并注入语感克隆规则（句式节奏/词汇偏好/感官密度/情绪幅度），**优先级最高，高于内置风格模块**
