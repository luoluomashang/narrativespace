...
description: |
  ...
  基于三位一体·叙事空间创作系统 v8.5，将原来的8个并行Skill统一打包成1个。
  十二步工作流 + 动态知识库 + 帮回辅助系统 + 去AI痕迹后处理 + 行文DNA采集 + TRPG沉浸互动 + 记忆锚点防遗忘 + RAG本地向量检索 + 卷级时间线里程碑追踪 + 跨模型一致性 + 全局叙事DNA系统。
metadata:
  version: 8.5.0
  edition: unified
  skills_included: 8
...
- **全局叙事DNA系统**：跨项目持久化作者风格偏好（词汇黑名单/句式偏好/价值观底线/节奏偏好），三层优先级链（cycle_quirks > dna_human_* > global_author_dna），项目级规则自动覆写全局基底
- **风格路由器 `/style` 指令**：三作用域（global/project/cycle）自然语言风格调整，JSON 文件中转消除 Shell 转义问题
- **DNA记忆蒸馏**：`consolidate_dna.py` 六阶段提纯（语义去重/冲突检测/权重衰减/Token预算强制执行），Ollama 余弦相似度防极性盲区
- **卷末一键提纯**：`volume_snapshot.py --consolidate-dna` 触发 DNA 提纯

### v8.4 新增（2026-04-05）

- **人味规则扩展 ht_08~ht_12**：记忆碎片/日常宏大对比/哲理隐喻落地/间接人性刻画/母题回响
- **导演模式（丁类帮回）**：用户以导演身份控制场景、NPC和节奏，无需中断状态机
- **动态节拍突变**：DM sub-agent 可主动制造节奏反转（急刹/情绪反转/节奏坠落）
- **跨模型一致性**：T1/T2/T3 三层约束体系 + 输出格式锚点（cross_model_consistency.yaml）
- **KB 自愈校验**：scene_plan 与 kb_diff 自动对比，5条以内自愈补丁 + WARNING提示
- **Humanizer 豁免额度**：R1/R3 规则每章允许不超过3个"有意保留"的豁免标注
- **LANDING 人味增强**：落盘前主进程快检 ht_08~ht_12 密度，不足时自动添加≤80字补强段
- **冲突解决方案**：wr_11/ht_01、ht_03/感官密度、ht_07/旁白风格等跨规则冲突已明确仲裁
