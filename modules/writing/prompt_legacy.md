...
**动态风格模块加载（每章必须执行，优先级从低到高：active_modules < clone < dna_human < cycle_quirks）**：
- 读取 `state.json → style_module_state.active_modules`，若列表非空，逐一加载 `.xushikj/config/style_modules/{module}.yaml`，提取规则摘要追加到 `write_constraints`（优先级 medium，高于通用 style_rules.yaml）
- 扫描 `.xushikj/config/style_modules/` 目录，若存在任意 `dna_human_*.yaml` 文件，加载并注入行文DNA可执行约束（DO/DON'T 对照表 + 标杆段落 + 8 微观基因段约束），**优先级 supreme，高于所有其他风格文件（含 clone_*.yaml）**
- 扫描 `.xushikj/config/style_modules/` 目录，若存在任意 `clone_*.yaml` 文件，加载并注入克隆语感规则（句式节奏/词汇偏好/感官密度），**优先级 high，低于 dna_human_*.yaml，高于 style_modules 内置模块**
...
```
优先读取 state.json → benchmark_state.linked_author + style_library_path：
  → 若 linked_author 存在，读取 {style_library_path}/{linked_author}/manifest.yaml（默认 ~/.narrativespace/style_library/{linked_author}/manifest.yaml）
  → 按 scene_type 匹配切片；无精确匹配时回退 daily

若全局库不可用或无匹配：
  → 从 .xushikj/benchmark/style_snippets/ 匹配最接近的 1-2 个切片文件
  → 匹配优先级：精确 scene_type 匹配 > daily 通用回退
...
| `dna_constraints` | write_constraints 内含（dna_human_*.yaml） | 行文DNA可执行约束——DO/DON'T 对照表 + 标杆段落 + 8 微观基因段约束（v9.0 升级，优先级 supreme，高于 clone_*.yaml） |
...
如果 state.json → style_module_state.active_modules 非空：
  ...
  → 提取规则摘要注入 write_constraints（优先级 medium，高于 style_rules.yaml）

扫描 .xushikj/config/style_modules/dna_human_*.yaml（v9.0，**supreme** 优先级）：
  → 若存在，加载行文DNA可执行约束（DO/DON'T 对照表 + 标杆段落 + 8 微观基因段约束）
  → 注入 write_constraints，**优先级 supreme，高于 clone_*.yaml 和所有内置模块**
  → 若与 global_author_dna.yaml 存在相同 category+id 规则，全局规则自动让位

扫描 .xushikj/config/style_modules/clone_*.yaml（优先级 high）：
  → 若存在，加载克隆指令集（句式节奏/词汇偏好/感官密度/情绪幅度）
  → 注入 write_constraints，优先级 high（低于 dna_human_*.yaml，高于内置风格模块）
...
加载 .xushikj/style_logs/cycle_quirks.md（v8.5 本卷短期覆写层）：
  ...
  → 覆写优先级：cycle_quirks（本层）> dna_human_*（supreme）> clone_*（high）> active风格模块（medium）> global_author_dna（foundation）> 内置规则
...
按 `state.json.config.reply_length` 使用最小中文字符数（整数）作为硬门槛：

1. 默认值：2000
2. 推荐区间：2000-5000
3. 用户可按项目目标设置（示例：2500/4000/5000）
4. 统计方式：仅统计中文字符（统一基线）
...
```bash
python scripts/chinese_char_count.py --input .xushikj/chapters/chapter_{N}.md
```

将输出记为 `zh_char_count`，与 `state.json.config.reply_length` 对比：
- 若 `zh_char_count < reply_length`：立即 HALT 严格打回；禁止进入 KB diff、summary、state 推进；禁止提供“强制通过/强制落盘”选项。
...
2. 中文字数门禁（强制）：
  - 执行 `python scripts/chinese_char_count.py --input .xushikj/chapters/chapter_{N}.md`
  - 读取 `state.json.config.reply_length` 作为最小中文字符数门槛
  - 若 `zh_char_count < reply_length`：HALT 严格打回，回到重写流程（不得强制落盘）
3. 检查返回状态：
   - HALT → 向用户展示选项，等待决策
   - PASS/WARN → 进入写后自查闸门
4. 执行写后自查：
  ...
  - HALT → 向用户展示选项，等待决策
5. 验证并应用 KB diff → 增量更新 knowledge_base.json
6. 更新伏笔追踪（如有 plant/resolve）
6.5. 【v8.0 新增】生成记忆锚点：
...
7. 启动章节概括 sub-agent（model="haiku"）→ 等待返回
8. 质量评估 → 读取章节文件 → 输出报告
9. 更新 state.json：
