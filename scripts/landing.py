...
def _upsert_markdown_section(sections: list[tuple[str, str]], heading: str, body: str) -> list[tuple[str, str]]:
    updated = False
...
def _update_memory(
    ...
    sections = _upsert_markdown_section(
        ...
        '当前写作硬约束',
        '\n'.join([
            f'- 最小中文字符数：{reply_length if reply_length > 0 else "（待确认）"}',
            f'- 目标平台（可选）：{target_platform or "（未设置）"}',
        ]),
    )
    sections = _upsert_markdown_section(
        sections,
...
def land_writing(project_dir: Path, input_file: Path, chapter: int | None) -> int:
    ...
    _update_memory(
        ...
        hook_items,
        reply_length,
        str(state.get('target_platform') or ''),
    )
    ...
    mark_step_complete(
        project_root,
        step='10',
        output_paths=[chapter_path, summary_path, memory_path, structured_path],
        validation_passed=True,
        validation_summary=f'chapter_{chapter_no} 中文字数={count}',
        next_step_suggestion='humanizer',
    )
