...
按 `state.json.config.reply_length` 执行最小中文字符数（整数）：

1. 默认值：`2000`
2. 推荐区间：`2000-5000`
3. 用户可自定义具体门槛（例如 2500/4000/5000）
4. 字数统计基线：仅统计中文字符（使用 `scripts/chinese_char_count.py`）

验收与落盘强制规则：
1. 章节验收与 LANDING 落盘前，必须执行 `scripts/chinese_char_count.py` 获取 `zh_char_count`
2. 必须比较 `zh_char_count` 与 `state.json.config.reply_length`
3. 若 `zh_char_count < reply_length`：立即 HALT 严格打回，禁止 KB diff / summary / state 推进
4. 不允许“强制通过/强制落盘”绕过字数门禁
